...
int main (int argc, char *argv[]) {
    ...
    for (i = 1; i < H; i++)
        for(j = 0; j < W; j++)
            matDct1[i][j] = sqrt(2./(double)(H)) * cos(M_PI * (2*j + 1) * i / (2 * H)); // Convention à définir

    double **matDct2 = alocamd(W, H);
    for (i = 0; i < H; i++) {
        matDct2[i][0] = 1/sqrt(W);
        for (j = 1; j < W; j++)
            matDct2[i][j] = sqrt(2./(double)(W))*cos(M_PI*(2*i+1)*j/(2*W)); // Convention à définir (voir 132)
    }

    // À définir: c'est quoi?
    /*
    for (i = 0; i < H; i++)
        for (j = 0; j < W; j++)
